package com.aits.careesteem.view.switchbutton.view;
...
public class SwitchButton extends CompoundButton {
    ...
    private float mTrackStrokeWidth;
    private Paint mTrackStrokePaint;

    private float mCustomTrackWidth = 0;
    ...
    private void init(AttributeSet attrs) {
        ...
        // Initialize the new paint for track stroke
        mTrackStrokePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTrackStrokePaint.setStyle(Paint.Style.STROKE); // Set style to STROKE
        ...
        float trackStrokeWidth = 0; // Default stroke width
        int trackStrokeColor = Color.TRANSPARENT; // Default stroke color (transparent)

        float customTrackWidth = 0;
        ...
        if (ta != null) {
            ...
            trackStrokeColor = ta.getColor(R.styleable.SwitchButton_kswTrackStrokeColor, trackStrokeColor);
            trackStrokeWidth = ta.getDimension(R.styleable.SwitchButton_kswTrackStrokeWidth, trackStrokeWidth);
            customTrackWidth = ta.getDimension(R.styleable.SwitchButton_kswTrackWidth, 0); // Get kswTrackWidth
            ta.recycle();
        }
        ...
        mTrackStrokeColor = trackStrokeColor;
        mTrackStrokeWidth = trackStrokeWidth;
        mTrackStrokePaint.setColor(mTrackStrokeColor);
        mTrackStrokePaint.setStrokeWidth(mTrackStrokeWidth);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Example: Drawing stroke after drawing the colored background
        if (!mIsBackUseDrawable && mTrackStrokeWidth > 0 && mTrackStrokeColor != Color.TRANSPARENT) {
            // You might need to adjust the mBackRectF slightly if you want the stroke
            // to be perfectly outside or inside the filled track.
            // For an outer stroke, you might not need to adjust if mBackRectF is the fill area.
            // For an inner stroke, you'd inset mBackRectF by mTrackStrokeWidth / 2.

            // Assuming mBackRadius is the corner radius for the track
            if (mBackRadius > 0) {
                canvas.drawRoundRect(mBackRectF, mBackRadius, mBackRadius, mTrackStrokePaint);
            } else {
                canvas.drawRect(mBackRectF, mTrackStrokePaint);
            }
        }
